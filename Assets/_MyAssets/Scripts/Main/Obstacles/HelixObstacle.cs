...
namespace DashSync.Main
{
    ...
    public class HelixObstacle : MonoBehaviour, IObstacle
    {
        ...
        [SerializeField]
        private float helixRevolutionCount;
        ...
        public void DrawObstacle(LODinfo info)
        {
            ...
            var deltaAngle = 360f * helixRevolutionCount / info.resolution;
            var delta = helixLength / info.resolution;
            for (int i = 0; i < info.resolution; i++)
            {
                //Calculate position of point around the ring
                ...
                var point = new Vector3(xPos, yPos, i * delta);
